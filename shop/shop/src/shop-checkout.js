                  ...
                  <dom-repeat items="[[cart]]" as="entry">
                    <template>
                      <div class="row order-summary-row">
                        <div class="flex">[[entry.item.title]] ( Qty: [[entry.quantity]])</div>
                        <div>[[_getEntryTotal(entry)]]</div>
                      </div>
                    </template>
                  </dom-repeat>
                  <div class="row total-row">
                    <div class="flex"><h2>Total</h2></div>
                    <div><h2>[[_formatPrice(total)]]</h2></div>
                  </div>
...
let orderData="*Hello SHomes Furnishing* %0a Following products are selected",totalAmount=0;this.cart.forEach(cartItem=>{orderData=`${orderData} %0a ${cartItem.item.name} - Quantity : ${cartItem.quantity}`;totalAmount=totalAmount+cartItem.quantity*cartItem.item.price;ga("send","event",cartItem.item.category,"checkout",cartItem.item.name,{dimension1:this.$.customerName.value,// Custom Dimension 1 - Name
dimension2:this.$.accountPhone.value// Custom Dimension 2 - Phone Number
})});orderData+=`%0a *Total Amount - ₹${totalAmount}*`;orderData+="%0a Confirm me once order placed";//console.log( this.$.accountPhone.value)
